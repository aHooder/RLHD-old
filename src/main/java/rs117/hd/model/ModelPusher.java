	...
	private int[] getFaceVertices(
		...
		if (isTextured) {
			...
			if (!plugin.configUndoVanillaShadingInCompute) {
				...
				float nx, ny, nz, lightDotNormal, lighten;
				...
				if (modelOverride.flatNormals || xVertexNormals == null || yVertexNormals == null || zVertexNormals == null) {
					...
					if (lightDotNormal > 0) {
						...
						lighten = color1L < IGNORE_LOW_LIGHTNESS ? 0 : color1L - IGNORE_LOW_LIGHTNESS;
						color1L += (lighten * LIGHTNESS_MULTIPLIER + BASE_LIGHTEN - color1L) * lightDotNormal;
						lighten = color2L < IGNORE_LOW_LIGHTNESS ? 0 : color2L - IGNORE_LOW_LIGHTNESS;
						color2L += (lighten * LIGHTNESS_MULTIPLIER + BASE_LIGHTEN - color2L) * lightDotNormal;
						lighten = color3L < IGNORE_LOW_LIGHTNESS ? 0 : color3L - IGNORE_LOW_LIGHTNESS;
						color3L += (lighten * LIGHTNESS_MULTIPLIER + BASE_LIGHTEN - color3L) * lightDotNormal;
					}
				} else {
